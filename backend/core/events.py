...
class EventBus:
    ...
    def add_listener(self, event_pattern: str, callback: Callable,
                     priority: int = 0, source: str = "", once: bool = False):
    ...
    def remove_all_from_source(self, source: str):
        ...
        for pattern in list(self._handlers.keys()):
            self._handlers[pattern] = [
                h for h in self._handlers[pattern] if h.source != source
            ]
        self._wildcard_handlers = [
            h for h in self._wildcard_handlers if h.source != source
        ]
