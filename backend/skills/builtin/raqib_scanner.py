...
VULN_PATTERNS = {
    "python": [
        ...
        (
            r'\.format\s*\(.*\)|f["\'].*\{.*\}',
            "Potential format string injection in SQL",
            "CWE-89",
            "info",
...
CONFIG_CHECKS = [
    ...
    {
        "file": "Dockerfile",
        "pattern": r"(?i)(?:ENV|ARG)\s+(?:.*PASSWORD|.*SECRET|.*KEY)=",
...
class RaqibScannerSkill(SkillBase):
    ...
    async def scan_secrets(self, params: dict) -> dict:
        ...
        return {"findings": findings, "files_scanned": files_scanned, "scan_type": "secrets"}
